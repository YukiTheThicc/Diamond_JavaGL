...
public class StateMachine extends Component {
    ...
    private class StateTrigger {
        ...
        public boolean equals(Object obj) {
            ...
            StateTrigger t = (StateTrigger) obj;
    ...
    public void update(float dt) {
        if (currentState != null) {
            ...
            SpriteRenderer sprite = parent.getComponent(SpriteRenderer.class);
            if (sprite != null) {
    ...
    public void imgui() {
        for (AnimationState state : states) {
            ...
            ImBoolean loops = new ImBoolean(state.doesLoop());
            ImGui.checkbox("Loops?", loops);
            state.setLoops(loops.get());
            int i = 0;
            for (Frame frame : state.frames) {
                float[] tmp = new float[1];
                tmp[0] = frame.time;
                ImGui.dragFloat("Frame(" + i + ") Time: ", tmp, 0.01f);
                frame.time = tmp[0];
                i++;
            }
        }
